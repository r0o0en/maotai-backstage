...
if(typeof layui == 'object'){
	...
	form.verify({
		...
		goodsClassName:function(val){
			if(!/^[\u4e00-\u9fa5a-zA-Z0-9-_]+$/ig.test(val)){
				return '商品名称长度至少唯一（允许中文、数字、字母、下划线)'
				console.log(arguments);
			}
		},
...
/*
 非登录页判断token
* */
if(!/login\.html/ig.test(location.pathname) && !getCookie('token')){
	toLoginPage();
}
...
function ajax(opt) {
	...
	if(!/login\.html/ig.test(location.href)){
		...
		if(getCookie('token')){
			...
			setCookie('power', getCookie('power'));
		}else{
			toLoginPage();
		}
	}
...
//					parent.location.href = location.origin + "/maotai-backstage/login.html";
//				}, 1500);
			} else {
		...
		if(/login\.html/ig.test(location.href)){
			...
			setCookie('power', data.data.power);			
		}
